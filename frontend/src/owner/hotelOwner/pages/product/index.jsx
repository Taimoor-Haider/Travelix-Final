...
import HotelOwnerBookings from "../HotelOwnerBookings";
import { useNavigate } from "react-router-dom";
function Index() {
  const { userInfo } = useSelector(loginSelector);
  const { ownerBookings } = useSelector(hotelListSelector);
  ...
  return (
    <div>
      <nav className="w-full flex items-center justify-center gap-4 mt-8 mb-8">
        ...
        <Link className={linkClasses("hotels")} to={"/product/hotels"}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
      ...
      {subpage === "hotels" && <HotelPage />}
      {subpage === "bookings" && <HotelOwnerBookings />}
    </div>
  );
}
